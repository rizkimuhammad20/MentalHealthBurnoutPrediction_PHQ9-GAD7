# Mental Health Burnout Prediction 🧠💼

A machine learning project designed to predict the likelihood or risk of mental health burnout in individuals or employees based on various behavioral and work-related features. 

## 📊 Key Features
* **Exploratory Data Analysis (EDA):** Visualized relationships between work conditions (e.g., working hours, stress levels, work-life balance) and burnout rates.
* **Feature Engineering:** Crafted and transformed new meaningful variables from the existing dataset to capture deeper patterns and improve the predictive power of the model.
* **Data Preprocessing:** Handled missing values, encoded categorical variables, and scaled numerical features for optimal model performance.
* **Machine Learning Modeling:** Implemented a **Logistic Regression** model for efficient, robust, and interpretable classification.
* **Feature Importance:** Analyzed the coefficients of the Logistic Regression model to identify exactly which work/lifestyle factors contribute the most to mental health burnout.

## 🛠️ Tech Stack
* **Language:** Python
* **Machine Learning:** Scikit-learn
* **Data Manipulation & Engineering:** Pandas, NumPy
* **Visualization:** Matplotlib, Seaborn

## 🚀 Getting Started

### Prerequisites
Ensure you have Python 3.8+ installed.

### Installation
1. Clone the repository:
   ```bash
   git clone [https://github.com/rizkimuhammad20MentalHealthBurnoutPrediction_PHQ9-GAD7.git](https://github.com/rizkimuhammad20MentalHealthBurnoutPrediction_PHQ9-GAD7.git)
   cd MentalHealthBurnoutPrediction_PHQ9-GAD7
